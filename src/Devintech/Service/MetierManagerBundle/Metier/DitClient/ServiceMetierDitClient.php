...
namespace App\Devintech\Service\MetierManagerBundle\Metier\DitClient;
...
class ServiceMetierDitClient
{
     ...
     * @return array
     */
    public function getAllDitClient()
    {
     ...
     * @return array
     */
    public function getDitClientById($_id)
    {
     ...
     * @return boolean
     */
    public function saveDitClient($_client, $_action)
    {
        if ($_action == 'new') {
     ...
     * @return boolean
     */
    public function deleteDitClient($_client)
    {
     ...
     * @return boolean
     */
    public function deleteGroupDitClient($_ids)
    {
        if (count($_ids)) {
            foreach ($_ids as $_id) {
                $_client = $this->getDitClientById($_id);
                $this->deleteDitClient($_client);
            }
        }
     ...
     * @return boolean
     */
    public function saveDitClientInOrder($_data)
    {
        // Récupérer manager
        ...
        if ($_is_email_exist) {
            ...
            $_user_role = $_user_role_manager->getDitRoleById(RoleName::ID_ROLE_CLIENT);
            $_user->setDitRole($_user_role);
