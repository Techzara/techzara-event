...
namespace App\Devintech\Service\MetierManagerBundle\Metier\DitMessageNewsletter;
...
class ServiceMetierMessageNewsletter
{
    ...
    {
        return $this->_entity_manager->getRepository(EntityName::DIT_MESSAGE_NEWSLETTER);
    }
     ...
     * @param DitMessageNewsletter $_message_newsletter
    ...
    public function insertFrontMessageNewsletter($_message)
    {
        ...
        $_message_newsletter = new DitMessageNewsletter();
    ...
    public function sendEmailNewsletter($_request, $_emails)
    {
        ...
        $_subject = $_post['dit_service_metiermanagerbundle_message_newsletter']['translations']['fr']['messageNewsletterTitle'];
        $_content = $_post['dit_service_metiermanagerbundle_message_newsletter']['translations']['fr']['messageNewsletterContent'];

        $_template = 'AdminBundle:DitMessageNewsletter:email.html.twig';
        ...
        foreach($_emails as $_email) {
            if ($_email->isNwsSubscribed() == true) {
                ...
                $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
                $_message    = (new \Swift_Message('DevInTech | ' . $_subject))
