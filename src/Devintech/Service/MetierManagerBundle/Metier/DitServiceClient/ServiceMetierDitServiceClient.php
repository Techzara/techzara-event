...
namespace App\Devintech\Service\MetierManagerBundle\Metier\DitServiceClient;
...
class ServiceMetierDitServiceClient
{
    ...
    {
        return $this->_entity_manager->getRepository(EntityName::DIT_SERVICE_CLIENT);
    }
     ...
     */
    public function getNbDitServiceClientQueryBuilder($_options = null)
    {
     ...
     * @return mixed
     */
    public function getAllDitServiceClientBy($_options, $_filters=array(), $_sortings=array())
    {
     ...
     * @return mixed
     */
    public function getNbDitServiceClientBy($_options=array(), $_filters = array())
    {
        $_query = $this->getNbDitServiceClientQueryBuilder($_options);
    ...
    {
        $_entity = EntityName::DIT_SERVICE_CLIENT;
        $_qb     = $this->_entity_manager
            ->createQueryBuilder('sc')
            ->select('sc')
            ->from($_entity, 'sc')
            ->leftJoin('sc.ditUser', 'usr')
            ->leftJoin('sc.ditService', 'srv');
     ...
     * @return array
     */
    public function getAllDitServiceClient()
    {
        // Récupérer l'utilisateur connecté
        $_user_connected = $this->_container->get('security.token_storage')->getToken()->getUser();
        $_id_user        = $_user_connected->getId();
     ...
     * @return array
     */
    public function getAllDitServiceClientNotValidated()
    {
     ...
     * @return array
     */
    public function getAllDitServiceClientValidated()
    {
        return $this->getRepository()->findBy(
     ...
     * @return array
     */
    public function getDitServiceClientById($_id)
    {
     ...
     * @return boolean
     */
    public function addDitServiceClient($_service_client, $_piece_jointes)
    {
        // Récupérer manager
        ...
        return $this->saveDitServiceClient($_service_client, 'new');
    }
     ...
     * @return boolean
     */
    public function updateDitServiceClient($_service_client, $_piece_jointes)
    {
        // Récupérer manager
        $_service_client_jointe_manager = $this->_container->get(ServiceName::SRV_METIER_SERVICE_CLIENT_JOINTE);
     ...
     * @return boolean
     */
    public function saveDitServiceClient($_service_client, $_action)
    {
        if ($_action == 'new') {
     ...
     * @return boolean
     */
    public function deleteDitServiceClient($_service_client)
    {
        // Récupérer manager
        $_service_client_jointe_manager = $this->_container->get(ServiceName::SRV_METIER_SERVICE_CLIENT_JOINTE);

        $_id_service_client      = $_service_client->getId();
        $_service_client_jointes = $_service_client_jointe_manager->getDitServiceClientJointeByServiceClient($_id_service_client);
     ...
     * @return boolean
     */
    public function deleteGroupDitServiceClient($_ids)
    {
        // Récupérer manager
        $_service_client_jointe_manager = $this->_container->get(ServiceName::SRV_METIER_SERVICE_CLIENT_JOINTE);
        ...
        if (count($_ids)) {
            foreach ($_ids as $_id) {
                $_service_client = $this->getDitServiceClientById($_id);
                ...
                $this->deleteDitServiceClient($_service_client);
            }
        }
    ...
    public function saveServiceClient($_client, $_data)
    {
        ...
        $_service                    = $_service_manager->getDitServiceById($_id_service);

        $_service_client = new DitServiceClient();
        ...
        $_service_client->setDitUser($_client);
        $_service_client->setDitService($_service);
        ...
        foreach ($_ids_service_option as $_id_service_option) {
            $_service_option = $_service_option_manager->getDitServiceOptionById($_id_service_option);
            $_service_client->addDitServiceOption($_service_option);
        }
     ...
     * @return array
     */
    public function setPaymentPayed(DitServiceClient $_service_client, $_payed)
    {
        $_service_client->setSrvCltIsPayed($_payed);

        $this->saveDitServiceClient($_service_client, 'update');
    }
     ...
     * @return boolean
     */
    public function sendEmailBonCommandePayment(DitServiceClient $_service_client)
    {
        // Récupérer l'adresse mail destinataire
        $_recipient = $_service_client->getDitUser()->getEmail();

        $_template   = 'AdminBundle:DitServiceClient:email_bon_commande.html.twig';
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Bon de commande'))
     ...
     * @return boolean
     */
    public function sendEmailNouveauCommande(DitServiceClient $_service_client)
    {
        ...
        $_template   = 'AdminBundle:DitServiceClient:email_nouveau_commande.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'service_client' => $_service_client,
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Nouveau commande'))
            ->setFrom(array($_from_email_address => $_from_firstname))
     ...
     * @return boolean
     */
    public function sendEmailCancelValidation(DitServiceClient $_service_client, $_comment)
    {
        // Récupérer l'adresse mail destinataire
        $_recipient = $_service_client->getDitUser()->getEmail();

        $_template   = 'AdminBundle:DitServiceClient:email_cancel_validation.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'service_client' => $_service_client,
            'comment'        => $_comment
        ));
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Fichiers non conformes'))
            ->setFrom(array($_from_email_address => $_from_firstname))
            ->setTo($_recipient)
            ->setBody($_email_body);
     ...
     * @return array
     */
    public function cancelStatusValidationService(DitServiceClient $_service_client, $_comment)
    {
     ...
     * @return array
     */
    public function setStatusValidationService(DitServiceClient $_service_client, $_status)
    {
     ...
     * @return array
     */
    public function setStatusProjectService(DitServiceClient $_service_client, $_status)
    {
    ...
    public function sendEmailConnexionClient(User $_user)
    {
        ...
        $_template   = 'AdminBundle:DitServiceClient:email_connexion_client.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'user'          => $_user,
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Lien espace membre'))
            ->setFrom(array($_from_email_address => $_from_firstname))
            ->setTo($_recipient)
            ->setBody($_email_body);
     ...
     * @return array
     */
    public function isUserValid(DitServiceClient $_service_client)
    {
        // Récupérer l'utilisateur connecté
        $_user_connected = $this->_container->get('security.token_storage')->getToken()->getUser();
        $_id_user        = $_user_connected->getId();
        $_id_role_user   = $_user_connected->getDitRole()->getId();
     ...
     * @return boolean
     */
    public function sendEmailClientByStatus(DitServiceClient $_service_client, $_id_status)
    {
        // Récupérer l'adresse mail destinataire
        $_recipient = $_service_client->getDitUser()->getEmail();
        ...
        $_template   = 'AdminBundle:DitServiceClient:email_client.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'service_client' => $_service_client,
            'message'        => $_message
        ));

        $_subject            = 'Devintech | ' . EtatServiceValidation::$VALEUR_TYPE[$_id_status];
        $_from_email_address = $this->_container->getParameter('from_email_address');
        $_from_firstname     = $this->_container->getParameter('from_firstname');
     ...
     * @return array
     */
    public function finaliseStatusValidationService(DitServiceClient $_service_client, $_lien_code_source)
    {
     ...
     * @return array
     */
    public function lienLivreStatusValidationService(DitServiceClient $_service_client, $_lien_livre)
    {
     ...
     * @return boolean
     */
    public function sendEmailLienLivreValidation(DitServiceClient $_service_client, $_lien_livre)
    {
        // Récupérer l'adresse mail destinataire
        $_recipient = $_service_client->getDitUser()->getEmail();

        $_template   = 'AdminBundle:DitServiceClient:email_lien_livre_validation.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'service_client' => $_service_client,
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Lien livré'))
            ->setFrom(array($_from_email_address => $_from_firstname))
            ->setTo($_recipient)
            ->setBody($_email_body);
     ...
     * @return boolean
     */
    public function sendEmailFinaliseValidation(DitServiceClient $_service_client, $_lien_code_source)
    {
        // Récupérer l'adresse mail destinataire
        $_recipient = $_service_client->getDitUser()->getEmail();

        $_template   = 'AdminBundle:DitServiceClient:email_finalise_validation.html.twig';
        $_email_body = $this->_container->get('templating')->renderResponse($_template, array(
            'service_client'   => $_service_client,
        ...
        $_email_body = implode("\n", array_slice(explode("\n", $_email_body), 4));
        $_message    = (new \Swift_Message('Devintech | Projet finalisé'))
            ->setFrom(array($_from_email_address => $_from_firstname))
            ->setTo($_recipient)
            ->setBody($_email_body);
