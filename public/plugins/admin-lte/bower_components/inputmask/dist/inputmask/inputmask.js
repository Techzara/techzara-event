...
!function(factory) {
    ...
    function maskScope(actionObj, maskset, opts) {
        ...
        function determineTestTemplate(tests, guessNextBest) {
            for (var testPos, testPositions = tests, lvp = getLastValidPosition(), ditTest = getMaskSet().validPositions[lvp] || getTests(0)[0], ditTestAltArr = ditTest.alternation !== undefined ? ditTest.locator[ditTest.alternation].toString().split(",") : [], ndx = 0; ndx < testPositions.length && (testPos = testPositions[ndx],
            !(testPos.match && (opts.greedy && !0 !== testPos.match.optionalQuantifier || (!1 === testPos.match.optionality || !1 === testPos.match.newBlockMarker) && !0 !== testPos.match.optionalQuantifier) && (ditTest.alternation === undefined || ditTest.alternation !== testPos.alternation || testPos.locator[ditTest.alternation] !== undefined && checkAlternationMatch(testPos.locator[ditTest.alternation].toString().split(","), ditTestAltArr))) || !0 === guessNextBest && (null !== testPos.match.fn || /[0-9a-bA-Z]/.test(testPos.match.def))); ndx++) ;
            return testPos;
        }
        ...
        function checkVal(input, writeOut, strict, nptvl, initiatingEvent) {
            ...
            if ($.each(inputValue, function(ndx, charCode) {
                if (charCode !== undefined) {
                    ...
                    var lvp = getLastValidPosition(undefined, !0), ditTest = getMaskSet().validPositions[lvp], nextTest = getTestTemplate(lvp + 1, ditTest ? ditTest.locator.slice() : undefined, lvp);
        ...
        function determineLastRequiredPosition(returnDefinition) {
            var pos, testPos, buffer = getBuffer(), bl = buffer.length, lvp = getLastValidPosition(), positions = {}, ditTest = getMaskSet().validPositions[lvp], ndxIntlzr = ditTest !== undefined ? ditTest.locator.slice() : undefined;
            for (pos = lvp + 1; pos < buffer.length; pos++) testPos = getTestTemplate(pos, ndxIntlzr, pos - 1), 
            ndxIntlzr = testPos.locator.slice(), positions[pos] = $.extend(!0, {}, testPos);
            var ditTestAlt = ditTest && ditTest.alternation !== undefined ? ditTest.locator[ditTest.alternation] : undefined;
            for (pos = bl - 1; pos > lvp && (testPos = positions[pos], (testPos.match.optionality || testPos.match.optionalQuantifier && testPos.match.newBlockMarker || ditTestAlt && (ditTestAlt !== positions[pos].locator[ditTest.alternation] && null != testPos.match.fn || null === testPos.match.fn && testPos.locator[ditTest.alternation] && checkAlternationMatch(testPos.locator[ditTest.alternation].toString().split(","), ditTestAlt.toString().split(",")) && "" !== getTests(pos)[0].def)) && buffer[pos] === getPlaceholder(pos, testPos.match)); pos--) bl--;
        ...
        function clearOptionalTail(buffer) {
            for (var validPos, rl = determineLastRequiredPosition(), bl = buffer.length, lv = getMaskSet().validPositions[getLastValidPosition()]; rl < bl && !isMask(rl, !0) && (validPos = lv !== undefined ? getTestTemplate(rl, dit.locator.slice(""), lv) : getTest(rl)) && !0 !== validPos.match.optionality && (!0 !== validPos.match.optionalQuantifier && !0 !== validPos.match.newBlockMarker || rl + 1 === bl && "" === (lv !== undefined ? getTestTemplate(rl + 1, dit.locator.slice(""), lv) : getTest(rl + 1)).match.def); ) rl++;
